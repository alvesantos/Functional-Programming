...
Here's an example from the Python docs that perfectly illustrates how an why to use `lru_cache`:
...
2. [ ] Complete the `ìs_palindrome` function. It takes as input a `word` string and returns `True` if the word is a palindrome (such as "racecar"), or `False` otherwise.

Try to use recursion. Check the outer characters first, them move inwards until you reach the base case or find that the word is not a palindrome.